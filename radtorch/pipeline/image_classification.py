...
############# BETA STARTS HERE ###########################################################################################
            if self.dim_reduction:
                ...
                test_pca = decomposition.PCA(n_components=self.red_dim)
                train_pca.fit(self.train_feature_extractor.features)
                test_pca.fit(self.test_feature_extractor.features)
                self.red_train_features=pd.DataFrame(train_pca.transform(self.train_feature_extractor.features))
                self.red_test_features=pd.DataFrame(test_pca.transform(self.test_feature_extractor.features))
                self.extracted_feature_dictionary={
            ...
            if self.feature_selection:
                ...
                if self.feature_selection_mode=='variance':
                    ...
                    self.selected_train_features=self.train_feature_extractor.features[self.selected_features_names]
                ...
                elif self.feature_selection_mode in ['kbest', 'fpr']:
                    if self.feature_selection_mode == 'kbest':
                        self.feature_selector=feature_selection.SelectKBest(feature_selection.chi2, **self.feature_selection_params)
                    elif self.feature_selection_mode=='fpr':
                        self.feature_selector=feature_selection.SelectFpr(feature_selection.chi2, **self.feature_selection_params)
                    self.selected_train_features=pd.DataFrame(self.feature_selector.fit_transform(self.train_feature_extractor.features, self.train_feature_extractor.labels_idx))
                    self.selected_features_names=(np.array(self.train_feature_extractor.feature_names)[self.feature_selector.get_support().tolist()]).tolist()
                    self.selected_test_features=self.test_feature_extractor.features[self.selected_features_names]
                    self.selected_train_features=self.train_feature_extractor.features[self.selected_features_names]
